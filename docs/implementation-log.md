...
8. **增量任务同步**：后续发现新的结构性问题时，可以追加本地 task 文件并再同步成新的 GitHub issue（本次追加了 #10）
...
**新增重构：query-scoped storage**

在实际测试时发现一个结构性问题：不同关键词检索到的文献会被写入同一组全局目录（`data/search-results/`, `data/papers/`, `data/papers-md/`, `data/notes/`, `outputs/`），这会让多个主题的综述运行互相污染。

因此新增了 GitHub task `#10`：`Refactor: query-scoped storage for literature review runs`。
